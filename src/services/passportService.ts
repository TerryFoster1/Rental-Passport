...
function createDemoPassportSummary(userId: string): PassportSummary {
  ...
  const sections = mergeSectionRows(
    passportSectionDefinitions.map((definition) => ({
      section_key: definition.key,
      ...initialSectionState[definition.key],
      last_updated_at: definition.key === 'employment' ? now : null,
    ...
    activity: [
      {
        ...
        description: 'Passport framework created.',
        visibility: 'internal',
        created_at: now,
      },
      {
        ...
        description: 'Employment section started.',
        visibility: 'internal',
        created_at: now,
      },
    ],
